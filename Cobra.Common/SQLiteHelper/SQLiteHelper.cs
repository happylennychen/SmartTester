    ...
    public class SQLiteHelper
    {
        ...
        public DataTable GetTableList()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if(!t.ToLower().StartsWith("sqlite_"))
                    dt2.Rows.Add(t);
            }
            return dt2;
        }

        public UInt32 GetTableSize(string tableName, Dictionary<string, string> dic_attchments = null)
        {
            UInt32 wval = 0;
            DataTable dt = null;
            StringBuilder strB = new StringBuilder();
            strB.Append(string.Format("SELECT Count(*) FROM(`{0}`) ", tableName));
            if ((dic_attchments != null) && (dic_attchments.Count != 0))
            {
                strB.Append("WHERE ");
                foreach (KeyValuePair<string, string> att in dic_attchments)
                {
                    strB.Append(string.Format("{0} = '{1}' COLLATE NOCASE ", att.Key, att.Value));
                    strB.Append("AND ");
                }
                strB.Length -= "AND ".Length;
            }
            dt = Select(strB.ToString());
            if (UInt32.TryParse(dt.Rows[0][0].ToString(), out wval))
                return wval;
            else
                return 0;
        }
        public DataTable GetTableData(string tableName,UInt32 saddr, UInt32 offset,Dictionary<string,string> dic_attchments = null)
        {
            StringBuilder strB = new StringBuilder();
            strB.Append(string.Format("SELECT * FROM(`{0}`) ", tableName));
            if ((dic_attchments == null)|(dic_attchments.Count == 0))
            {
                strB.Append(string.Format("LIMIT {1},{2};", tableName, saddr, offset));
                return Select(strB.ToString());
            }
            else
            {
                strB.Append("WHERE ");
                ...
                strB.Length -= "AND ".Length;
                strB.Append(string.Format("LIMIT {0},{1};", saddr, offset));
                return Select(strB.ToString());
            }
        }
